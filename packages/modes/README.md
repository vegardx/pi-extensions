...
|------|-------|------|-------------|
| `plan` | read-only (`read`, `bash`, `grep`, `find`, `ls`, `websearch`, `webfetch`, `plan_step`) | blocked if write-capable | none — writes are refused outright |
...
- **From plan** — if a plan exists (steps or assistant text): show the picker (Implement / Park / Continue discussing). Otherwise cycle directly to ask.
- **From ask** → auto
- **From auto** → plan
